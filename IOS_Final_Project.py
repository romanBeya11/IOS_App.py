...
class MyScene (Scene):
	...
	def increment_high_score(self):
		if self.coin_intersect == self.contents_of_high_score:
			self.high_score.text = "High Score: " + str(self.coin_intersect)
			self.hs = open('High_Score_IOS_Final_Game', 'w+')
			self.hs.write('' + str(self.coin_intersect))
			self.hs.close()
	
	def retrieve_high_score(self):
		self.retrieve_hs = open('High_Score_IOS_Final_Game', 'r')
		if self.retrieve_hs.mode == 'r':
			self.contents_of_high_score = self.retrieve_hs.read()
			self.high_score.text = 'High Score: ' + str(self.contents_of_high_score)
			#print self.contents_of_high_score
	...
	def create_new_coin(self):
		...
		self.new_coin_position.x = 100
		self.new_coin_position.y = 100
		self.new_coin = SpriteNode('plf:Item_CoinGold', position = self.new_coin_position, parent = self.coin, size = self.size / 13) #/15
	
	def move_coin(self):
		random_coin_coordinate_x = random.randint(self.left_side_of_screen_boundary, self.right_side_of_screen_boundary)
		random_coin_coordinate_y = random.randint(self.height_of_ground, self.score_label.y - 30)
		if self.coin_position.x <= self.size.x:
			self.coin.run_action(Action.move_to(random_coin_coordinate_x, random_coin_coordinate_y, 5)) 
	...
	def coin_to_player_intersection(self):
		...
		if self.player.frame.contains_point(self.coin.frame):
